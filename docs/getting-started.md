...
A profile is a named bundle of runtime feature flags selected by `MAGI_RUNTIME_PROFILE`. When no profile is set, a clean install now boots the **`lab`** tier by default: the full governance/harness modules plus the complete experimental flag set. Set `MAGI_RUNTIME_PROFILE=full` for the leaner stable tier (standard governance/harness, experimental set off), or one of the lean profiles (`safe`, `eval`, `minimal`, `conservative`, `off`) to keep the runtime at its conservative code defaults. Registry code defaults are unchanged, so a library/test import stays conservative regardless.
...
### `lab` — experimental dogfood tier (now the default)

`lab` is the `full` profile plus the complete experimental flag set (extra verification gates, the learning loop, memory, deep web research, document-coverage in non-blocking `advisory` mode, and more). It is now the default tier when no profile is set, so you usually do not need to name it. To pin it explicitly (or to get a clean `lab` file), make `~/.magi/profile.env` a single line:
