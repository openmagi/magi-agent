...
|---|---|---|
| **Stable-intent** | `EvidenceContract` / `EvidenceRequirement` / `EvidenceRecord` / `EvidenceSource` (`magi_agent.evidence.types`), the `BUILTIN_EVIDENCE_TYPES` catalog + `custom:PascalCaseName` namespace, `HarnessEngine` construction and `resolve()` contracts + `EvidenceContractScope`, harness JSON schema (`docs/harness-schema.md`) | Breaking changes only at a minor release, with a CHANGELOG entry, a deprecation alias kept for >=1 minor release, and a migration note in `docs/upgrading.md`. |
| **Evolving** | Recipe pack manifest schemas, tool manifest schemas (`magi_agent/tools/manifest.py`), public runtime env-flag names documented in `docs/env-reference.md` | May change at any minor release; renames ship with a one-release alias when mechanically possible. Flag removals are listed in the CHANGELOG. |
