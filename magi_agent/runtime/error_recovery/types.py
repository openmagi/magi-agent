...
class ErrorRecoveryConfig(BaseModel):
    ...
    def from_env(cls) -> ErrorRecoveryConfig:
        """Construct config from the environment via the canonical reader.

        Delegates to config.env.parse_error_recovery_env so this constructor
        can never drift from the registered profile-bool contract
        (default-ON full profile, OFF under safe/eval).
        """
        from magi_agent.config.env import parse_error_recovery_env  # noqa: PLC0415

        parsed = parse_error_recovery_env(os.environ)
        return cls(
            recovery_enabled=parsed.enabled,
