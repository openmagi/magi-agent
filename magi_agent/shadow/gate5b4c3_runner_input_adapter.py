    ...
    *,
    ...
    if request.recipe_profile.tools_policy == "selected_full_toolhost":
        base = (
            ...
            "patch, calculation, search, or workspace work, or when answering requires "
            "facts you do not reliably hold, such as the contents of installed skills, "
            "workspace files, or session artifacts. Questions about an installed skill, "
            "file, or this workspace are workspace work: read the artifact first, then "
            "answer. Installed skills live under the workspace skills/ directories as "
            "SKILL.md files; the SkillLoader tool lists and loads them. When the user "
            "names a skill (for example with a leading /skill-name) or asks you to "
            "describe or run one, load its SKILL.md with SkillLoader or FileRead before "
            "answering; never describe a skill from memory. For brief replies, do not "
            ...
            "not a valid completion. For greetings, thanks, acknowledgements, or small "
            "talk, reply directly and briefly, and never narrate task status, completion "
            "checks, or goal audits for a turn that contained no task. When the user "
            "gives you an actual task: do not finish by promising future or background "
            ...
            "execute it. Once the task is fully done, give a final text answer stating "
            "the concrete results, or clearly state a concrete blocker. The answer "
            "itself is the completion: state what was done and stop; never append "
            "meta-commentary about goals being met, requests being satisfied, or "
            "nothing being outstanding. Never claim that this response, mode, or "
            "configuration prevents you from calling tools, reading files, or taking "
            "an action when the tool list for this turn includes it. If you choose not "
            "to use a tool, either use it or state your actual reason; do not attribute "
            "the choice to a nonexistent restriction. Do not claim broad production workspace "
