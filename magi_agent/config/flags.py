...
FLAGS: tuple[FlagSpec, ...] = (
    ...
    _b(
        ...
        summary=(
            ...
            "shell_exec) wired at the live ADK before_tool_callback boundary "
            "via magi_agent/cli/customize_tool_wiring.py (engine "
            "_attach_customize_rules), sharing the same stage helper consumed "
            "by magi_agent/facades.py:execute_tool_with_hooks; and "
    ...
    _b(
        ...
        summary=(
            ...
            "script exits non-zero; wired at the live ADK "
            "before_tool_callback boundary via "
            "magi_agent/cli/customize_tool_wiring.py (engine "
            "_attach_customize_rules), sharing the same stage helper "
            "consumed by magi_agent/facades.py:execute_tool_with_hooks); "
            "``after_tool_use`` (audit-only, dispatch already "
            "returned; same live ADK after_tool_callback boundary); plus 9 "
            "lifecycle_audit fan-out "
    ...
    _b(
        ...
        summary=(
            ...
            "two gate slots (``pre_final`` and ``before_tool_use``, the "
            "latter wired at the live ADK before_tool_callback boundary via "
            "magi_agent/cli/customize_tool_wiring.py sharing the same stage "
            "helper consumed by magi_agent/facades.py:execute_tool_with_hooks) "
            "as a deterministic-shaped verdict source: the operator-authored "
    ...
    _b(
        ...
        summary=(
            ...
            "Wired at the live ADK after_tool_callback boundary via "
            "magi_agent/cli/customize_tool_wiring.py (engine "
            "_attach_customize_rules), sharing the same stage helper consumed "
            "by magi_agent/facades.py:execute_tool_with_hooks (parallel to the "
