...
| watchers.py | Watcher-fleet builders — COMPOSE the existing always-on blocks (Track F). | _truthy, channel_watchers, child_runner_live, daemon, driver, flags, goal_judge, notifier, runner, scheduler_job_execution, scheduler_job_store, scheduler_loop_driver, store, turn_engine | cli/app.py, gateway/channel_watchers.py, gateway/daemon.py, runtime/durable_recovery.py |
