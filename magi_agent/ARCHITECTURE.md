...
| channel_message_store.py | Durable per-channel message log for local-serve multi-window history sync. | flags, migrations, session_store | transport/chat_routes_local.py |
...
| streaming_chat_route.py | Hosted-grade SSE streaming-chat HTTP surface. | active_turn, app_api, chat, chat_shared, child_runner_live, contracts, env, events, flags, gate5b_full_toolhost, goal_loop_policy, health, local_session_registry, local_turn_pump, local_turn_store, memory_mode_context, modes, per_turn_goal_intensity, per_turn_goal_loop_context, protocol, providers, public_events, serve_token, session_store, skill_slash, streaming_chat, streaming_driver, streaming_sink, usage_cost, wiring | (root)/app.py, cli/tests/test_model_picker_wire.py |
