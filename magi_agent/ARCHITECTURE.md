...
| goal_loop_policy.py | Runtime ``GoalLoopPolicy`` — the per-turn shape PR-C's clean-break judge | — | runtime/per_turn_goal_loop_context.py, transport/chat_routes.py |
...
| per_turn_goal_loop_context.py | Per-turn ``ContextVar`` carrying the active :class:`GoalLoopPolicy`. | goal_loop_policy | transport/chat_routes.py |
