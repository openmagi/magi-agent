...
| facades.py | High-level entry-point facades that compose existing modules. | bus, context, dispatcher, flags, manifest, prompt_injection, replace_payloads, resolved, result, store, verification_policy | — |
...
| custom_rules.py | Custom verification-rule schema + validation (spec §9.1). | capability_scope, prompt_injection, shacl_verifier, what_menu | customize/rule_compiler.py, transport/customize.py |
...
| replace_payloads.py | Typed payload schemas for ``HookResult(action='replace', value=...)``. | manifest | (root)/facades.py |
