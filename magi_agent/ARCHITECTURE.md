...
| streaming_chat_route.py | Hosted-grade SSE streaming-chat HTTP surface. | active_turn, app_api, chat, child_runner_live, contracts, env, events, flags, gate5b_full_toolhost, goal_loop_policy, health, local_session_registry, local_turn_pump, local_turn_store, memory_mode_context, modes, per_turn_goal_intensity, per_turn_goal_loop_context, protocol, providers, public_events, session_store, streaming_chat, streaming_driver, streaming_sink, usage_cost, wiring | (root)/app.py, cli/tests/test_model_picker_wire.py |
