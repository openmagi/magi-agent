...
``learning_live_readiness``:
    ...
    * a frozen pydantic config whose single authority flag
      ...
      :func:`scheduler_executor_readiness_health_metadata` from the selected
...
enforce that the OSS in-process scheduler and the legacy external ``oc-cron`` are
...
class SchedulerExecutorReadinessConfig(BaseModel):
    ...
    from the selected scope and the promotion stage (mirrors
    ...
    if conflict:
        return {
            ...
            "reason": (
                ...
                "legacy external oc-cron (MAGI_OC_CRON_ACTIVE=1) are active simultaneously — "
...
def check_oc_cron_transition_guard_from_env() -> dict[str, object]:
    ...
    Reads:
      ...
      MAGI_OC_CRON_ACTIVE              — legacy external oc-cron active (default OFF).
    """
