...
def _hosted_serve_gate_refusal(runtime: object) -> JSONResponse | None:
    """Completions-equivalent refusal for hosted serving over the stream route.
    ...
    fallback. Returns ``None`` only when the selected gate5b canary gate is
    fully active. Never falls through to the local headless engine.
    """
    if os.environ.get("CORE_AGENT_PYTHON_CHAT_ROUTE", "off").lower() != "on":
        config = getattr(runtime, "config", None)
        return JSONResponse(
...
def register_streaming_chat_routes(
    ...
    *,
    ...
    async def streaming_chat_stream(request: Request) -> Response:
        ...
        if _hosted_streaming_serve_active():
            ...
            refusal = _hosted_serve_gate_refusal(runtime)
