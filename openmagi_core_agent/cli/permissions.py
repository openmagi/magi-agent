...
(``openmagi_core_agent.cli.contracts`` and
``openmagi_core_agent.runtime.control``) so importing it never pulls in
``textual`` / ``google-adk``.
...
from openmagi_core_agent.cli.protocol import ControlRequestFrame, ControlResponse
...
class FrameWriter(Protocol):
    ...
    Satisfied by :class:`openmagi_core_agent.cli.ndjson.NdjsonWriter` and by any
