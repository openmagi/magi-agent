...
_LEGACY_IDENTITY_PATTERNS: tuple[tuple[re.Pattern[str], str, str], ...] = (
    (
        re.compile(r"\bclawy\s+agent\b", re.IGNORECASE),
        "Magi Agent",
        "legacy_public_identity_normalized",
    ),
    (
        re.compile(r"\bclawy-core-agent\b", re.IGNORECASE),
        "OpenMagi runtime",
        "legacy_runtime_identity_normalized",
    ),
    (
        re.compile(r"\bclawy\b", re.IGNORECASE),
        "OpenMagi",
        "legacy_public_identity_normalized",
    ),
)
    ...
    *,
    ...
    try:
        generation = _build_user_visible_generation_request(
            ...
            payload=payload,
            trace_id=request.headers.get("x-clawy-trace-id"),
