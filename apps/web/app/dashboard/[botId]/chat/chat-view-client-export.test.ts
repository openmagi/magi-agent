...
describe("bot-scoped ChatViewClient export wiring", () => {
  it("wires selected-message export to markdown download and public link creation", () => {
    ...
    expect(source).toContain("t.chat.createPublicLinkDescription");
    expect(source).toContain("onExportSelected={handleExportSelected}");
  });

  it("wires the channel header export button to open message selection", () => {
    const source = readFileSync(new URL("./chat-view-client.tsx", import.meta.url), "utf8");

    expect(source).toContain("const handleStartExportSelection = useCallback(() => {");
    expect(source).toContain("store.startSelectionMode(activeChannel);");
    expect(source).toContain("aria-label={t.chat.startExportSelection}");
    expect(source).toContain("{t.chat.exportConversation}");
  });

  it("renders channel memory mode text in the chat header", () => {
    const source = readFileSync(new URL("./chat-view-client.tsx", import.meta.url), "utf8");

    expect(source).toContain("formatChannelBaseLabel");
    expect(source).toContain("formatChannelMemoryLabel");
    expect(source).toContain("const activeChannelTitle = useMemo(() => {");
    expect(source).toContain("const activeChannelMemoryLabel = useMemo(() => {");
    expect(source).toContain("{activeChannelTitle}");
    expect(source).toContain("{activeChannelMemoryLabel}");
  });
  ...
  it("clears selected KB context after an accepted send while keeping failed sends selected", () => {
    const source = readFileSync(new URL("./chat-view-client.tsx", import.meta.url), "utf8");

    expect(source).toContain("const clearAcceptedKbContext = useCallback(() => {");
    expect(source).toMatch(/if \(result\.injected\) \{[\s\S]*clearAcceptedKbContext\(\);[\s\S]*return true;/);
    expect(source).toMatch(/if \(!ok\) \{[\s\S]*return false;[\s\S]*\}\s*clearAcceptedKbContext\(\);\s*return true;/);
    expect(source).toMatch(/void performSend\(text, activeReply, messageKbDocs,[\s\S]*\)\.catch[\s\S]*clearAcceptedKbContext\(\);\s*return true;/);
  });
