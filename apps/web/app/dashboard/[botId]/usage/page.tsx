...
// Pricing display removed for OSS — agent runs on user's own API keys

type Period = "7d" | "30d" | "90d";

const MODEL_KEYS = ["haiku", "sonnet", "opus", "kimi", "minimax", "gpt_5_nano", "gpt_5_mini", "gpt_5_1", "gpt_5_5", "gpt_5_5_pro", "gemini_3_1_flash", "gemini_3_1_flash_lite", "gemini_2_5_flash", "gemini_2_5_pro", "gemini_3_1_pro", "codex"] as const;
...
const MODEL_COLORS: Record<ModelKey, string> = {
  ...
  gemini_3_1_flash_lite: "text-lime-400",
  gemini_2_5_flash: "text-lime-400",
  gemini_2_5_pro: "text-orange-400",
  gemini_3_1_pro: "text-red-400",
...
const MODEL_DISPLAY_NAMES: Record<ModelKey, string> = {
  ...
  gemini_3_1_flash_lite: "Gemini 3.1 FL",
...
interface ConverterData {
  ...
  totalPages: number;
  totalCostCents: number;
  daily: { date: string; count: number; pages: number; costCents: number }[];
}
        ...
        return (
          <div key={day.date}>
            ...
            <button
              ...
              className="w-full flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-black/5 transition-colors text-left"
            >
            ...
            {isExpanded && day.hours && (
              <div className="ml-3 mb-2 border-l border-gray-200 pl-3">
                {day.hours.map((hr) => (
                  <div key={hr.hour} className="py-2 border-b border-gray-100 last:border-0">
  ...
  useEffect(() => {
    async function fetchBot() {
      try {
        ...
        setIsByok(bot.api_key_mode === "byok");
      } catch {
    ...
    fetchBot();
    fetchQuotaAndGrant();
  }, [authFetch, botId, t.errors.loadUsage]);
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            ...
            {usage.converter && usage.converter.totalJobs > 0 && (
              <GlassCard>
                <p className="text-sm text-secondary mb-1">Document Converter</p>
                <p className="text-2xl font-bold text-foreground">
                  {usage.converter.totalJobs}
                </p>
                <p className="text-xs text-muted mt-1">{usage.converter.totalPages} pages converted</p>
                {!isByok && usage.converter.totalCostCents > 0 && (
                  <p className="text-xs text-amber-400 mt-1">{formatCents(usage.converter.totalCostCents)} credits</p>
                )}
              </GlassCard>
            )}
              ...
              <GlassCard>
                ...
                {creditGrant.is_trialing ? (
                  <>
                    ...
                    <div className="w-full h-1.5 bg-black/5 rounded-full overflow-hidden mt-2">
                      <div className="h-full bg-emerald-600/60 rounded-full" style={{ width: "0%" }} />
                    </div>
          ...
          {/* Service usage (search, email, firecrawl, converter) daily breakdown */}
          {(serviceUsage || usage.firecrawl || usage.converter) && (() => {
            ...
            const convMap = new Map((usage.converter?.daily ?? []).map((d) => [d.date, d]));
            const allDates = [...new Set([
              ...svcSearchMap.keys(), ...svcEmailMap.keys(), ...fcMap.keys(), ...convMap.keys(),
              ...
              <GlassCard className="mb-6">
                ...
                <div className="space-y-0.5">
                  {allDates.map((date) => {
                    ...
                    const conv = convMap.get(date);
                    return (
                      <div key={date} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-black/5 transition-colors">
                        ...
                        <div className="flex items-center gap-4 text-xs">
                          ...
                          {conv && conv.count > 0 && (
                            <span className="text-purple-400">
                              {conv.count} conversions ({conv.pages}p)
                              {conv.costCents > 0 && <span className="text-amber-400 ml-1">({formatCents(conv.costCents)})</span>}
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <tbody>
                    {usage.daily.map((day) =>
                      MODEL_KEYS.filter((key) => day.models[key]).map((key, idx) => {
                        ...
                        return (
                          <tr key={`${day.date}-${key}`} className="border-b border-gray-100 last:border-0">
                            {idx === 0 ? (
                              <td
