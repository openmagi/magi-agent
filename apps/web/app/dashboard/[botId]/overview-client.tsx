...
export function DashboardOverview({ bot, sessionId, openOnboarding, subscriptionPlan }: DashboardOverviewProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("magi:open-add-bot", handler);

    // Check sessionStorage flag (set by sidebar when navigating from another page)
    const flag = sessionStorage.getItem("magi:open-add-bot");
    if (flag) {
      sessionStorage.removeItem("magi:open-add-bot");
      setModalOpen(true);
    }

    return () => window.removeEventListener("magi:open-add-bot", handler);
  }, []);
