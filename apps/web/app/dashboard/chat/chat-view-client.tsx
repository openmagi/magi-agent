...
import { buildMessageContentWithKbContext } from "@/chat-core";
...
export function ChatViewClient({
      ...
      chatApi
        ...
        .then((msgs) => {
          if (msgs.length > 0) {
            const mapped = mapServerMessages(msgs);
            if (!isCurrentBot()) return;
            // Merge with existing rather than replacing — avoids losing messages
            // that were already displayed and breaking sort relative to dividers
            const prev = useChatStore.getState().serverMessages[channel] ?? [];
            // Replace existing entries with updated content from server, keep the rest
  ...
  // Core send path — same as [botId] version; the public `handleSend`
  // below enqueues instead during streaming.
        ...
        try {
          await chatApi.sendMessage(botId, channel, allMessages, {
            ...
            onResponseClear: () => {
              ...
              const prevText = currentState?.streamingText || "";
              store.setChannelState(channel, {
                streamingText: prevText,
                heartbeatElapsedMs: null,
                pendingInjectionCount: 0,
                subagents: [],
                documentDraft: null,
            ...
            onError: async (err) => {
              ...
              if (hasVisibleContent) {
                // Keep partial content for debugging/context, but mark it as
                // incomplete so it cannot look like a successful final answer.
                store.setChannelState(channel, {
                  error: err.message,
                  turnPhase: "aborted",
                }, { botId });
  ...
  const handleSend = useCallback(
    async (text: string, files?: File[], sendOptions?: ChatInputSendOptions) => {
      ...
      void performSend(messageText, uploadedKbDocs, undefined, sendOptions).catch((err) => {
  ...
  const handleChannelSelect = useCallback(
    (name: string) => {
      ...
      router.push(`/dashboard/chat/${encodeURIComponent(name)}`);
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [router],
  );
  ...
  const handleCreateChannel = useCallback(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [botId, handleChannelSelect, isCurrentBot],
  );
  ...
  const handleDeleteChannel = useCallback(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [botId, handleChannelSelect, isCurrentBot, router],
  );
  ...
  return (
    <div className="flex h-full bg-background overflow-hidden">
      <ChatSidebar
        ...
        onRenameChannel={() => { /* legacy chat route — rename not wired */ }}
        onRenameCategory={() => { /* legacy chat route — rename not wired */ }}
      />
      <div
      ...
      {undoData && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4 fade-in duration-200">
          <div className="flex items-center gap-3 bg-white border border-black/[0.08] text-foreground rounded-xl px-4 py-3 shadow-lg">
