...
export function CustomToolModal({ botId, open, onClose }: CustomToolModalProps): React.ReactElement | null {
  ...
  const handleToggle = async (toolName: string, currentEnabled: boolean): Promise<void> => {
    ...
    await fetch(`/api/bots/${botId}/tools/${encodeURIComponent(toolName)}/${action}`, { method: "PUT" });
  ...
  const handleDelete = async (toolName: string): Promise<void> => {
    const res = await fetch(`/api/bots/${botId}/tools/${encodeURIComponent(toolName)}`, { method: "DELETE" });
  ...
  const handleAddTool = async (): Promise<void> => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error ?? t.customize.toolAddError); }
      setFormName(""); setFormDesc(""); setFormBody(""); setShowForm(false); setFetched(false); void fetchTools();
    } catch (err) { setFormError(err instanceof Error ? err.message : t.customize.toolAddError); } finally { setFormSaving(false); }
  };

  const SOURCE_LABEL: Record<string, string> = { builtin: t.customize.toolsSourceBuiltin, skill: t.customize.toolsSourceSkill, external: t.customize.toolsSourceExternal };
    ...
    <Modal open={open} onClose={onClose}>
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-1">
          <h2 className="text-lg font-semibold text-foreground">{t.customize.toolModalTitle}</h2>
        ...
        <p className="text-xs text-secondary mb-6">{t.customize.toolModalDesc}</p>

        {/* Add form */}
        {showForm ? (
          <div className="glass rounded-2xl p-5 mb-5 !border-primary/15 space-y-4">
            <Input label={t.customize.toolFormName} value={formName} onChange={(e) => setFormName(e.target.value)} placeholder={t.customize.toolFormNamePlaceholder} />
            <Input label={t.customize.toolFormDesc} value={formDesc} onChange={(e) => setFormDesc(e.target.value)} placeholder={t.customize.toolFormDescPlaceholder} />
            <Textarea label={t.customize.toolFormBody} value={formBody} onChange={(e) => setFormBody(e.target.value)} placeholder={t.customize.toolFormBodyPlaceholder} rows={5} className="!font-mono !resize-y" />
            {formError && <p className="text-xs text-red-500">{formError}</p>}
            <div className="flex items-center gap-3 pt-1">
              <Button variant="ghost" size="sm" onClick={() => { setShowForm(false); setFormError(null); }}>{t.customize.toolFormCancel}</Button>
              <Button variant="cta" size="sm" onClick={() => void handleAddTool()} disabled={formSaving || !formName.trim() || !formDesc.trim()}>
                {formSaving ? t.customize.toolFormSaving : t.customize.toolFormAdd}
              </Button>
            </div>
          </div>
        ) : (
          <Button variant="secondary" className="!w-full !border-dashed !border-2 mb-5" onClick={() => setShowForm(true)}>
            + {t.customize.toolAddNew}
          </Button>
        )}
        ...
        {loading ? (
          <div className="flex flex-col items-center py-10">
            ...
            <p className="text-xs text-secondary mt-3">{t.customize.toolsLoading}</p>
          </div>
        ) : (
          <div className="space-y-5">
            {externalTools.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs font-semibold text-foreground">{t.customize.toolCustomSection}</p>
            ...
            {otherTools.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs font-semibold text-foreground">{t.customize.toolSystemSection}</p>
                {otherTools.map((tool) => (
            ...
            {tools.length === 0 && fetched && (
              <div className="flex flex-col items-center py-10">
                ...
                <p className="text-sm text-secondary">{t.customize.toolEmpty}</p>
              </div>
            )}
          </div>
        )}
