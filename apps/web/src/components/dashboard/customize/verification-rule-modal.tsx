...
export function VerificationRuleModal({ botId, initialRules, initialAgentConfig, open, onClose }: VerificationRuleModalProps): React.ReactElement | null {
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      const res = await agentFetch(`/v1/settings`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ agent_rules: rules, ...(presetDirty ? { agent_config: { builtin_presets: presetConfigs } } : {}) }) });
