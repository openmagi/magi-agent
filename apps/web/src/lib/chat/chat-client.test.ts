...
describe("fetchChannels", () => {
  ...
  it("falls back to the legacy chat proxy when the Open Magi proxy is unreachable", async () => {
    ...
    expect(String(fetchMock.mock.calls[1]?.[0])).toBe("https://chat.openmagi.ai/v1/chat/bot-abc/channels");
  });
});

describe("sendMessage local runtime", () => {
  it("posts OSS local bot turns to the same-origin streaming endpoint", async () => {
    ...
    expect(String(fetchMock.mock.calls[0]?.[0])).toBe("/v1/chat/stream");
...
describe("interruptTurn", () => {
  ...
  it("posts OSS local cancellation to the streaming cancel endpoint", async () => {
    ...
    expect(r.handoffRequested).toBe(true);
    const [url, opts] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(url).toBe("/v1/chat/cancel");
    expect(opts.method).toBe("POST");
