...
# ── Helpers ──
# integration.sh is in $BIN_DIR which is in PATH

fetch_notion() {
  integration.sh "notion/$1" 2>/dev/null
}

fetch_google() {
  integration.sh "google/$1" 2>/dev/null
}
...
INTEGRATION_SH=$(command -v integration.sh)
export INTEGRATION_SH
...
INTEGRATION_SH = os.environ.get("INTEGRATION_SH", shutil.which("integration.sh") or "integration.sh")
...
for p in pages:
    ...
    try:
        result = subprocess.run(
            [INTEGRATION_SH, f"notion/page/{p['id']}"],
...
for db in databases:
    ...
    try:
        result = subprocess.run(
            [INTEGRATION_SH, f"notion/database/{db['id']}"],
            capture_output=True, text=True, timeout=30
        )
...
for sheet_id in all_gdrive_ids:
    try:
        result = subprocess.run(
            [INTEGRATION_SH, f"google/sheets-metadata?spreadsheetId={sheet_id}"],
            capture_output=True, text=True, timeout=30
        )
