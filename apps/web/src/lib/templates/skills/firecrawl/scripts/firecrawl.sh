...
URL="$2"
LIMIT="${3:-10}"
if [ -z "$ACTION" ] || [ -z "$URL" ]; then echo '{"error":"Usage: firecrawl.sh <scrape|crawl|map> <url> [limit]"}'; exit 1; fi
...
if [ -n "$FIRECRAWL_API_KEY" ]; then
  ...
  echo '{"error":"Firecrawl not available — no API key or platform proxy configured"}'; exit 1
fi
case "$ACTION" in
  scrape)
    RESULT=$(curl -sf --compressed -X POST "$API_BASE/scrape" \
      ...
      -d '{"url":"'"$URL"'","formats":["markdown"]}' 2>&1) || { echo '{"error":"Firecrawl scrape request failed"}'; exit 1; }
    echo "$RESULT" ;;
  crawl)
    RESULT=$(curl -sf --compressed -X POST "$API_BASE/crawl" \
      -H "$AUTH_HEADER" \
      -H "Content-Type: application/json" \
      -d '{"url":"'"$URL"'","limit":'"$LIMIT"'}' 2>&1) || { echo '{"error":"Firecrawl crawl request failed"}'; exit 1; }
    echo "$RESULT" ;;
  map)
    RESULT=$(curl -sf --compressed -X POST "$API_BASE/map" \
      -H "$AUTH_HEADER" \
      -H "Content-Type: application/json" \
      -d '{"url":"'"$URL"'"}' 2>&1) || { echo '{"error":"Firecrawl map request failed"}'; exit 1; }
    echo "$RESULT" ;;
  *) echo '{"error":"Unknown action: '"$ACTION"'. Use scrape, crawl, or map."}'; exit 1 ;;
esac
