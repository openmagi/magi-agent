...
description: "Build 5-level compaction tree (daily/weekly/monthly/root) with smart thresholds and fixed/tentative lifecycle. Normally runtime-managed; use this only for explicit recovery/debugging or external scheduler workflows."
user_invocable: true
---
...
Normally handled by the native Hipocampus runtime. Use this skill only when an operator explicitly needs recovery/debugging or when an external scheduler workflow still calls it. Check trigger conditions below.
