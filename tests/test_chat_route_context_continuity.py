...
def test_delivery_receipt_records_sanitized_continuity_diagnostics_only(
    monkeypatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("CORE_AGENT_PYTHON_CHAT_ROUTE", "on")
    counter_path = tmp_path / "counters.json"
    runtime = _runtime_with_continuity(
        mocked_runner=lambda _request: {"content": "must not run", "eventCount": 1},
        continuity=_continuity_config(),
    )
    ...
    response = TestClient(create_app(runtime)).post(
        ...
        json={
            ...
            "gate": "gate7_5_context_continuity",
            "deliveryStatus": "fallback_served",
            "reason": "python_error",
            "fallbackReason": "context_import_failed",
    ...
    continuity = record["contextContinuity"]
    assert continuity["schemaVersion"] == "pregate8.contextContinuityChatDiagnostic.v1"
    assert continuity["continuityEnabled"] is True
    assert continuity["importedEventCount"] == 3
    assert continuity["rejectedEntryCount"] == 1
...
def test_gate7_5_fallback_receipt_requires_selected_scope(
    monkeypatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("CORE_AGENT_PYTHON_CHAT_ROUTE", "on")
    ...
    assert response.json()["reason"] == "selected_scope_required"
    assert not counter_path.exists()


def test_delivery_receipt_rejects_client_supplied_continuity_metadata(
    monkeypatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("CORE_AGENT_PYTHON_CHAT_ROUTE", "on")
