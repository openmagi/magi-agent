...
def test_gate3b_metrics_does_not_import_runner_or_live_surfaces() -> None:
    completed = _run_fresh_python(
        """
import importlib
import sys

module = importlib.import_module("openmagi_core_agent.shadow.gate3b_metrics")
...
if loaded:
    raise AssertionError(f"Gate 3B metrics loaded forbidden modules: {loaded}")
"""
    )
...
def test_gate3b_metrics_source_has_no_shell_or_code_execution_helpers() -> None:
