...
def test_gate5b_import_is_schema_only_and_runtime_free() -> None:
    completed = _run_fresh_python(
        """
import importlib
import sys

module = importlib.import_module("openmagi_core_agent.shadow.gate5b_user_visible_routing_canary")
...
if loaded:
    raise AssertionError(f"Gate 5B import loaded forbidden modules: {loaded}")
"""
    )
...
def test_gate5b_source_forbids_runtime_routing_and_live_surfaces() -> None:
    root = Path(__file__).parents[1]
    module_path = (
        root
        / "openmagi_core_agent"
        / "shadow"
        / "gate5b_user_visible_routing_canary.py"
    )
    ...
    assert "runtime_selector_active: Literal[True]" not in source
    assert "user_visible_output_attached: Literal[True]" not in source
