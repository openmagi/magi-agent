...
def test_gate4c2_comparison_imports_no_runner_or_live_surfaces() -> None:
    completed = _run_fresh_python(
        """
import importlib
import sys

module = importlib.import_module("openmagi_core_agent.shadow.gate4c2_shadow_comparison_report")
...
if loaded:
    raise AssertionError(f"Gate 4C-2 comparison loaded forbidden modules: {loaded}")
"""
    )
...
def test_gate4c2_comparison_source_has_no_execution_or_runner_calls() -> None:
    root = Path(__file__).parents[1]
    module_path = (
        root / "openmagi_core_agent" / "shadow" / "gate4c2_shadow_comparison_report.py"
    )
