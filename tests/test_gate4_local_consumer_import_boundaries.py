...
def test_gate4_consumer_does_not_import_runner_model_or_live_surfaces() -> None:
    completed = _run_fresh_python(
        """
import importlib
import sys

module = importlib.import_module("openmagi_core_agent.shadow.gate4_consumer")
...
if loaded:
    raise AssertionError(f"Gate 4 consumer loaded forbidden modules: {loaded}")
"""
    )
...
def test_gate4_consumer_source_has_no_runner_model_shell_or_code_execution_helpers() -> None:
    root = Path(__file__).parents[1]
    module_paths = (
        root / "openmagi_core_agent" / "shadow" / "gate4_consumer.py",
        root / "openmagi_core_agent" / "shadow" / "gate4_bridge.py",
        root / "openmagi_core_agent" / "shadow" / "gate3b_local_consumer.py",
        root / "openmagi_core_agent" / "shadow" / "gate3b_local_report.py",
        root / "openmagi_core_agent" / "shadow" / "gate3b_metrics.py",
    )
    forbidden_imports = (
        "subprocess",
        "asyncio.subprocess",
