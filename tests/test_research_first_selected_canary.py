...
def test_research_first_selected_endpoint_returns_no_write_authority_or_live_model(
    monkeypatch,
    tmp_path,
) -> None:
    ...
    request_digest = _digest("research-first-request")
    env = _base_env(
        CORE_AGENT_PYTHON_GATE8_RESEARCH_FIRST_CANARY_ENABLED="1",
    ...
    assert body["researchFirst"]["finalGate"]["status"] == "passed"
    ...
    assert body["safety"]["schedulerMutationAllowed"] is False
    assert body["publicEvents"]
    assert "src_1" in body["choices"][0]["message"]["content"]
    assert body["counter"]["status"] == "research_first_completed"
    assert body["counter"]["state"]["dailyGenerationRunsUsed"] == 0
    ...
    assert request_record["status"] == "research_first_selected_readonly_completed"
    assert request_record["gate"] == "gate8_selected_python_authority"
    assert request_record["pythonCounterRecordPresent"] is True
    assert request_record["toolReceiptCount"] == 0
    assert request_record["modelAttemptCount"] == 0
    assert request_record["providerRequestCount"] == 0
    assert request_record["sourceLedgerDigest"] == body["researchFirst"]["sourceLedger"][
        "ledgerDigest"
    ]
    assert request_record["outputDigest"] == body["researchFirst"]["finalGate"][
        "finalAnswerDigest"
    ]

    receipt = TestClient(create_app(_runtime(env))).post(
        "/v1/internal/gate5b/user-visible-delivery-receipts",
        headers={"Authorization": "Bearer gateway-token"},
        json={
            "schemaVersion": "gate5b.userVisibleDeliveryReceipt.v1",
            "requestDigest": request_digest,
            "bodyDigest": _digest("body"),
            ...
            "sseFrameCount": len(body["publicEvents"]) + 3,
            "toolReceiptCount": 0,
            "modelAttemptCount": 0,
            "providerRequestCount": 0,
            "expectedModelAttemptCount": 0,
            "outputDigest": body["researchFirst"]["finalGate"]["finalAnswerDigest"],
            "sourceLedgerDigest": body["researchFirst"]["sourceLedger"]["ledgerDigest"],
            "finalProjectionDigest": body["researchFirst"]["finalGate"][
                "finalAnswerDigest"
            ],
            "researchEvidenceStatus": "passed",
            "citationEvidenceStatus": "passed",
            "verifierEvidenceStatus": "passed",
            "finalProjectionEvidenceStatus": "passed",
            "sourceInspectedEventCount": 1,
            "ruleCheckEventCount": 3,
            "unsupportedClaimOmittedCount": 0,
        },
    )
    assert receipt.status_code == 202, json.dumps(receipt.json(), sort_keys=True)
    assert receipt.json()["receiptStatus"] == "recorded"

    raw_counter_store = json.loads(counter_path.read_text(encoding="utf-8"))
    delivered_record = next(iter(raw_counter_store["scopes"].values()))["requests"][
        request_digest
    ]
    assert delivered_record["researchEvidenceStatus"] == "passed"
    assert delivered_record["citationEvidenceStatus"] == "passed"
    assert delivered_record["verifierEvidenceStatus"] == "passed"
    assert delivered_record["finalProjectionEvidenceStatus"] == "passed"
    assert delivered_record["sourceInspectedEventCount"] == 1
    assert delivered_record["ruleCheckEventCount"] == 3
    assert delivered_record["sourceLedgerDigest"] == body["researchFirst"]["sourceLedger"][
        "ledgerDigest"
    ]
    assert delivered_record["finalProjectionDigest"] == body["researchFirst"][
        "finalGate"
    ]["finalAnswerDigest"]
    assert "/Users/" not in json.dumps(delivered_record, sort_keys=True)
    assert "gateway-token" not in json.dumps(delivered_record, sort_keys=True)
    assert "Authorization" not in json.dumps(delivered_record, sort_keys=True)

    evidence = _runtime(env).gate5b4c3_shadow_generation_route_config.counter_store
    assert evidence is not None
    delivery_evidence = evidence.validate_delivery_evidence(
        request_digest=request_digest,
        selected_bot_digest=_digest("bot-research-first"),
        trusted_owner_user_id_digest=_digest("owner-research-first"),
        environment="production",
        gate="gate8_selected_python_authority",
    )
...
def test_research_first_delivery_receipt_requires_durable_evidence_categories(
    ...
    telemetry_path.write_text("", encoding="utf-8")
    counter_path = tmp_path / "gate8-research-first-counters.json"
    request_digest = _digest("research-first-missing-receipt-evidence")
    env = _base_env(
        CORE_AGENT_PYTHON_GATE8_RESEARCH_FIRST_CANARY_ENABLED="1",
        CORE_AGENT_PYTHON_GATE8_RESEARCH_FIRST_CANARY_KILL_SWITCH="0",
        CORE_AGENT_PYTHON_GATE5B_SHADOW_GENERATION_ENABLED="1",
    ...
    client = TestClient(create_app(_runtime(env)))
    ...
    assert receipt.status_code == 409, json.dumps(receipt.json(), sort_keys=True)
    assert receipt.json()["receiptStatus"] == "receipt_rejected"
    assert receipt.json()["reason"] == "research_first_evidence_missing"

    store = _runtime(env).gate5b4c3_shadow_generation_route_config.counter_store
    assert store is not None
    delivery_evidence = store.validate_delivery_evidence(
        request_digest=request_digest,
        selected_bot_digest=_digest("bot-research-first"),
        trusted_owner_user_id_digest=_digest("owner-research-first"),
        environment="production",
    ...
    assert delivery_evidence.status == "failed"
    assert delivery_evidence.reason == "research_first_delivery_not_served"


def test_research_first_fallback_receipt_cannot_satisfy_delivery_evidence(
    monkeypatch,
    tmp_path,
) -> None:
    telemetry_path = tmp_path / "egress.jsonl"
    telemetry_path.write_text("", encoding="utf-8")
    counter_path = tmp_path / "gate8-research-first-counters.json"
    request_digest = _digest("research-first-fallback-receipt")
    env = _base_env(
        CORE_AGENT_PYTHON_GATE8_RESEARCH_FIRST_CANARY_ENABLED="1",
        CORE_AGENT_PYTHON_GATE8_RESEARCH_FIRST_CANARY_KILL_SWITCH="0",
        CORE_AGENT_PYTHON_GATE5B_SHADOW_GENERATION_ENABLED="1",
    ...
    receipt = client.post(
        ...
        json={
            ...
            "reason": "python_call_failed",
            "responseAuthority": "typescript",
            "pythonAttempted": True,
            "pythonCounterRecordPresent": True,
            "fallbackReason": "python_call_failed",
            "sseFrameCount": 0,
            "toolReceiptCount": 0,
            "modelAttemptCount": 0,
            "providerRequestCount": 0,
