...
def test_public_deliver_skips_failed_non_empty_output() -> None:
    """The public delivery boundary must not deliver failed/timed-out output."""
    from magi_agent.harness.scheduler_delivery import LocalLogDeliverySink, deliver
    from magi_agent.harness.scheduler_job_execution import CronTurnResult

    result = CronTurnResult(
        status="failed",
        jobId="job:failed-public-delivery",
        runnerInvoked=True,
        output="partial failure text that must not be delivered",
    )

    receipt = deliver(result, target=LocalLogDeliverySink())

    assert receipt.status == "skipped"
    assert receipt.output_length == len(result.output)
