...
class TestNextRunAtCron:
    ...
    def test_invalid_cron_timezone_raises_not_none(self) -> None:
        """Invalid cron timezone must fail closed instead of looking exhausted."""
        from magi_agent.missions.schedule_grammar import next_run_at, parse_schedule
        ...
        with pytest.raises(ValueError, match="timezone"):
            next_run_at(spec, now=now, timezone="Not/AZone")

    def test_cron_without_next_run_raises_not_none(self) -> None:
        """A field-valid but unsatisfiable cron must not disable itself silently."""
        from magi_agent.missions.schedule_grammar import next_run_at, parse_schedule

        now = _utc(2026, 6, 3, 12, 0)
        spec = parse_schedule("0 0 31 2 *")

        with pytest.raises(ValueError, match="no next run|invalid cron"):
            next_run_at(spec, now=now)
