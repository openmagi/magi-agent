...
class TestSchedulerExecutorHealthProjection:
    ...
    def test_scheduler_executor_health_live_when_executor_on_shadow_off(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setenv("MAGI_SCHEDULER_EXECUTOR_ENABLED", "1")
...
class TestSchedulerExecutorReadinessImportPurity:
    def test_readiness_module_does_not_import_urllib(self) -> None:
        ...
        src = pathlib.Path(
            "/Users/kevin/Desktop/claude_code/magi-agent-track-A"
            "/magi_agent/gates/scheduler_executor_readiness.py"
        ).read_text()
        tree = ast.parse(src)
    ...
    def test_scheduler_metrics_module_does_not_import_network(self) -> None:
        ...
        src = pathlib.Path(
            "/Users/kevin/Desktop/claude_code/magi-agent-track-A"
            "/magi_agent/ops/scheduler_metrics.py"
        ).read_text()
        tree = ast.parse(src)
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
