...
def test_record_advance_false_blocks_receipt_and_fired_id(tmp_path: Any) -> None:
    """A failed/no-op advance must not be followed by a fired receipt."""
    from magi_agent.harness.scheduler_executor import InMemoryJobSource, tick

    class NoOpAdvanceSource(InMemoryJobSource):
        def record_advance(
            self,
            job_id: str,
            next_run: datetime,
    ...
    now = _now_dt(now_ms)
    lease = _make_lease(now_ms=now_ms)
    source = NoOpAdvanceSource(
        _make_source(
            [
                {
                    "job_id": "job:no-advance",
                    "schedule_expr": "every 10m",
                    "next_run_ms": now_ms - 500,
                }
            ]
    ...
    with pytest.raises(RuntimeError, match="record_advance"):
        tick(
            now=now,
            source=source,
            lease=lease,
            ...
            _on_receipt=lambda job_id, _receipt: receipts.append(job_id),
        )

    assert receipts == []


def test_record_advance_uses_expected_next_run_cas(tmp_path: Any) -> None:
    """A source must reject stale compare-and-set advances."""
    from magi_agent.harness.scheduler_executor import InMemoryJobSource, ScheduledJobRecord

    now_ms = 1_000_000
    old_next_run = datetime.fromtimestamp((now_ms - 500) / 1000, tz=UTC)
    source = InMemoryJobSource(
        [
            ScheduledJobRecord(
                jobId="job:cas",
                scheduleExpr="every 10m",
                lastFire=None,
                nextRun=old_next_run,
            )
        ]
    )

    accepted = source.record_advance(
        "job:cas",
        _now_dt(now_ms + 600_000),
        expected_next_run=old_next_run,
    )
    rejected = source.record_advance(
        "job:cas",
        _now_dt(now_ms + 1_200_000),
        expected_next_run=old_next_run,
    )
...
def test_tick_rejects_non_local_fake_source_by_default(tmp_path: Any) -> None:
    """A2 preview tick must not mutate arbitrary injected persistent sources."""
    from magi_agent.harness.scheduler_executor import ScheduledJobRecord, tick

    class PersistentLookingSource:
        def __init__(self) -> None:
            self.advanced = False
            self.record = ScheduledJobRecord(
                jobId="job:persistent",
                scheduleExpr="every 10m",
                lastFire=None,
                nextRun=_now_dt(999_500),
            )
    ...
    source = PersistentLookingSource()
    with pytest.raises(ValueError, match="local_fake"):
        tick(
            now=_now_dt(1_000_000),
            source=source,
            lease=_make_lease(now_ms=1_000_000),
            lock_dir=tmp_path,
            owner_digest="owner:test-abc",
        )
    assert source.advanced is False
...
def test_tick_lock_rejects_symlink_lock_file(tmp_path: Any) -> None:
    """The lock file must be opened no-follow so symlink swaps fail closed."""
    import os
    from magi_agent.harness.scheduler_executor import acquire_tick_lock
...
def test_tick_lock_rejects_shared_temp_lock_dir(tmp_path: Any) -> None:
    """Lock dirs under temp must still be private to the current user."""
    from magi_agent.harness.scheduler_executor import acquire_tick_lock

    lock_dir = tmp_path / "shared-locks"
    lock_dir.mkdir()
    old_mode = lock_dir.stat().st_mode & 0o777
