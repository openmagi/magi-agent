...
def test_gate5a_import_is_lazy_and_shadow_only() -> None:
    completed = _run_fresh_python(
        """
import importlib
import sys

module = importlib.import_module("openmagi_core_agent.shadow.gate5a_no_memory_shadow_canary")
...
if loaded:
    raise AssertionError(f"Gate 5A import loaded forbidden modules: {loaded}")
"""
    )
...
def test_gate5a_source_forbids_production_and_live_tool_surfaces() -> None:
    root = Path(__file__).parents[1]
    module_path = root / "openmagi_core_agent" / "shadow" / "gate5a_no_memory_shadow_canary.py"
