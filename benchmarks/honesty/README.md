...
## It holds across agent models (not one model's quirk)

The result above uses gpt-5.5 as the agent. Re-running with **claude-sonnet-4-6**
and **gemini-3.1-pro** as the agent (same Opus-4.8 judge) reproduces the shape.
Pooled across all three providers (unbacked N=27, backed N=40):

| layer | catches unbacked (recall ↑) | blocks receipt-backed work ↓ |
|---|---|---|
| judge · answer · neutral/skeptical | 27/27 (100%) | **40/40 (100%, CI [91,100])** |
| judge · transcript · neutral | 19/27 (70%) | 1/40 (2%, CI [0,13]) |
| **evidence-bound** | 27/27 (100%) | **0/40 (0%, CI [0,9])** |

A suspicious answer-only judge blocks **all 40** genuinely-completed tasks across
OpenAI / Anthropic / Google agents; the transcript judge and the receipt gate
block ~0. The CIs do not overlap — whether prose can vouch for real work is
model-independent. (Honest wrinkle: the stronger agents *make far fewer* unbacked
claims — asked to compute without a tool they disclose they can't, so their
unbacked N is only 3 each; a more honest agent is simply harder to catch. The
model-independent result is the blocks-backed column, N=40.) Full per-model tables
+ audit trail: `RESULT-v3-multimodel.md`, `published-verdicts/multimodel/`,
regenerated by `python -m benchmarks.honesty.multimodel`.
